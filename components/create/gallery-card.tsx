...
export function GalleryCard({ item }: GalleryCardProps) {
  ...
  const handleCopyPrompt = (e: React.MouseEvent) => {
    ...
    toast.success(t('copyToast.copiedLabel'), {
      description: t('copyToast.copiedDesc'),
    })
  }
  ...
  const handleRemix = (e: React.MouseEvent) => {
    ...
    window.location.href = `/create/?${queryParams.toString()}`
  }
  ...
  return (
    ...
    >
      <Link href="#" prefetch={false} className="block w-full cursor-zoom-in">
        {/* 
           关键修改：
      ...
      {/* Overlay - 保持之前的逻辑 */}
      <div
        className={`absolute inset-0 rounded-xl bg-gradient-to-t from-black/80 via-black/20 to-transparent transition-opacity duration-300 flex flex-col justify-end p-4 pointer-events-none ${
