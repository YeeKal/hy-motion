        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-2xl">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 py-6">
                    {/* Free Tier */}
                    <div className="space-y-4  p-5 rounded-xl bg-muted/50 border border-borderflex flex-col">
                        <div className="text-center space-y-1">
                            <h3 className="font-bold text-lg">{t('plans.free.title')}</h3>
                        </div>
                        <ul className="space-y-3 text-sm flex-1">
                            {freeFeatures.map((feature, index) => (
                                <li key={index} className="flex items-center gap-2">
                                    <Icon name={feature.icon} className={`w-4 h-4 flex-shrink-0 ${feature.icon === 'Check' ? 'text-green-500' : ''}`} />
                        ...
                        {!isLoggedIn && (
                            ...
                            //     </Button>
                            // </Link>
                            <ThirdPartyProviders callbackUrl="/create" isPop={true}></ThirdPartyProviders>
                        )}
                    </div>
